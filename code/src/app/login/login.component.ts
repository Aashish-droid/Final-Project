...
export class LoginComponent {
  UsersList:any;
  constructor(private as:ServicesService,private fb:FormBuilder, private router:Router){
    // validations
    this.signupform = this.fb.group({
      email:['',[Validators.required, Validators.pattern("^[A-Za-z][A-Za-z_\.0-9]+@[A-Za-z]+[\.][A-Za-z]{2,5}$")]],
      username: ['', [Validators.required, Validators.minLength(6), Validators.pattern("^[a-z0-9]+$")]],
    ...
    if (this.signupform.valid){
      ...
      if(this.existing!=true){
        ...
        this.as.newuser(data).subscribe(
          {
            next: (data:any)=>{
              alert("OPTIMUS PRIME WELCOMES YOU")
              location.reload()
            },
            error:()=>{
              alert("Some error")
